...
import { MAINNET_RPC, NETWORK, PROGRAM_ID } from '../config/constants';
...
export const getTokenAccount = async (wallet, connection, mintPk) => {
  ...
  console.log('[log] => tokenAccount: ', tokenAccount);
...
export const getNFTTokenAccount = async (wallet, connection, nftMintPk) => {
  ...
  let provider = await getProvider(wallet, connection);
  console.log('[log] => getNFTTokenAccount before query ');
  let tokenAccount = await provider.connection.getProgramAccounts(
    TOKEN_PROGRAM_ID,
    {
      filters: [
  ...
  console.log('[log] => NFTtokenAccount: ', tokenAccount);
...
export const getOwnerOfNFT = async (wallet, connection, nftMintPk) => {
  ...
  let tokenAccountInfo = await provider.connection.getAccountInfo(tokenAccountPK);
...
export const getNftMetadataURI = async (wallet, connection, nftMintPk) => {
  try {
    ...
    let metadataByte = await provider.connection.getAccountInfo(nftMetadataAddr);
